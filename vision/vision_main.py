...
class VisionMain:
    def __init__(self):
        ...
        # --- CROP CONFIGURATION (Adjust these values if needed) ---
        # Based on your logs: Real bottle is ~330, Noise is at ~590 and ~115(Y)
        # We crop a vertical strip around the bottle to ignore the rest.
    ...
    def run(self):
        ...
        try:
            while self.est_tournant:
                ...
                # --- ✂️ CROP THE FRAME (X-Axis Only) ---
                # Keep full height (:), crop width (start:end)
                frame_cropped = frame[:, self.crop_x_start:self.crop_x_end]
                
                # Optional: Visualize the crop area on the main frame for debugging
                cv2.rectangle(frame, (self.crop_x_start, 0), (self.crop_x_end, frame.shape[0]), (255, 0, 255), 1)

                # 1. Detection (Run on CROPPED frame)
                res_bouteille = self.detecteur_bouteille.detecter(frame_cropped, self.config)
                
                # --- 🔄 CORRECT COORDINATES ---
                # The detector returns coordinates relative to the cropped image (0 to 200).
                # We must add 'crop_x_start' to map them back to the original full image (250 to 450).
                ...
                if res_bouteille['present']:
                    # Pass the FULL frame to level checker, but with corrected BBOX
                ...
                # 4. Visualization (Draw on FULL frame)
                if res_bouteille['present']:
