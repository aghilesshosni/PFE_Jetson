...
class VisionMain:
    def __init__(self):
        ...
        self.frame_buffer = np.zeros(
            (self.config.hauteur, self.config.largeur, 3), dtype=np.uint8
        )
        self.est_tournant = False
        self.dernier_resultat = {
            'Autorisation_Remplissage': False, 'Arret_Convoyeur': False,
            'Defaut_Systeme': False, 'Status': 'Init', 'Compensation': 0,
            'Niveau_Remplissage': 0.0, 'Centre': None
        }

    def start(self):
        print("Initialisation du système de vision...")
        if not self.camera.open():
            raise RuntimeError("Échec critique : Impossible d'ouvrir la caméra")
        self.est_tournant = True
        print("Caméra active. Démarrage de la boucle...")
        return True
        ...
        try:
            while self.est_tournant:
                ...
                self.dernier_resultat.update({
                    'Autorisation_Remplissage': False, 'Arret_Convoyeur': False,
                    'Defaut_Systeme': False, 'Status': "AUCUNE_BOUTEILLE",
                    'Niveau_Remplissage': 0.0, 'Centre': None
                })
                ...
                if resultat['present']:
                    ...
                    if resultat_niveau.get('debordement', False):
                        status = "DEBORDEMENT"
                        self.dernier_resultat.update({'Autorisation_Remplissage': False, 'Arret_Convoyeur': True, 'Defaut_Systeme': True})
                    elif resultat_niveau.get('plein', False):
                        status = "BOUTEILLE_PLEINE"
                        self.dernier_resultat.update({'Autorisation_Remplissage': False, 'Arret_Convoyeur': False, 'Defaut_Systeme': False})
                    else:
                        status = "EN_REMPLISSAGE"
                        self.dernier_resultat.update({'Autorisation_Remplissage': True, 'Arret_Convoyeur': True, 'Defaut_Systeme': False})

                    self.dernier_resultat['Status'] = status
                    etat_actuel = f"{status} @ {centre}"
                    
                    if etat_actuel != self.dernier_etat_affiche:
                        print(f"{status} | Centre: {centre} | Niveau: {self.dernier_resultat['Niveau_Remplissage']:.1f}%")
                ...
                if temps_cycle < 0.033: time.sleep(0.033 - temps_cycle)
